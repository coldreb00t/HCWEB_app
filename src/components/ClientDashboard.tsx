...
export function ClientDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      await Promise.all([
        fetchNextWorkout(client.id),
        fetchWorkoutStats(client.id),
        fetchActivityStats(client.id),
        fetchMeasurementStats(client.id)
      ]);
      
      // Подсчет достижений должен идти после того, как другие данные загружены
      await fetchAchievementsStats(client.id);
  ...
  const fetchNextWorkout = async (clientId: string) => {
    try {
      const nextWorkout = await WorkoutService.getNextClientWorkout(clientId);
